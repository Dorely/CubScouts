# Paws for Action Adventure Lesson Plan

**Estimated Time to Complete: 2 den meetings (60 minutes each)**

## ✅ Requirements Checklist

### Meeting 1 Requirements:
- [ ] **Requirement 1:** Familiarize yourself with the flag of the United States of America including the history, demonstrating how to raise and lower the flag, how to properly fold and display, and the United States etiquette
- [ ] **Requirement 2:** Identify 3 symbols that represent the United States. Pick your favorite and make a model, work of art, or other craft that depicts the symbol
- [ ] **Requirement 3:** Learn about the mission of any non-profit. Find out how they fund their activities and how volunteers are used to help

### Meeting 2 Requirements:
- [ ] **Requirement 4:** Participate in a service project

---


## Meeting 1: American Pride & Service Learning (60 minutes)

### Opening (5 minutes)
- Opening ceremony with flag presentation
- Discuss what citizenship means

### Activities

#### Activity 1: Flag Education & American Symbols (25 minutes)
**Requirements 1 & 2: Learn about U.S. flag and identify 3 symbols**
- Quick flag trivia covering history, folding, and etiquette
- Practice flag ceremony with proper salutes
- Learn about American symbols: Statue of Liberty, White House, Eagle, Liberty Bell
- Choose favorite symbol and create quick craft (paint flag, draw symbol, or Play-Doh® creation)

#### Activity 2: Nonprofit Learning & Service Planning (25 minutes)
**Requirements 3 & 4: Learn about nonprofits and plan service project**
- Learn about local nonprofits: mission, funding, volunteers
- Interactive discussion about helping community
- Choose and plan service project:
  - Eyeglasses collection drive setup
  - Dog toy making for shelters
  - Care package assembly
  - Food drive organization

### Closing (5 minutes)
- Share symbol crafts and confirm service project plans

---

## Meeting 2: Service in Action (60 minutes)

### Opening (5 minutes)
- Review American values and service connection

### Activities

#### Activity 1: Service Project Execution (45 minutes)
**Requirement 4: Participate in service project**
- **Option A**: Create collection boxes and flyers for eyeglasses drive
- **Option B**: Make dog toys from old t-shirts for animal shelter
- **Option C**: Assemble hygiene kits with encouraging notes for homeless shelter
- **Option D**: Sort and package food items for food bank

#### Activity 2: Citizenship Reflection (5 minutes)
- Discuss how service connects to citizenship
- Share feelings about helping others
- Plan delivery or follow-up

### Closing (5 minutes)
- Celebrate service accomplishment and adventure completion

## Materials Needed
- U.S. flag for demonstrations
- Flag folding instruction guides
- Art supplies for symbol crafts:
  - Paint, brushes, paper
  - Play-Doh® or clay
  - Construction paper, markers
- Information about local nonprofits
- Service project specific materials:
  - Collection boxes and labels
  - Old t-shirts for dog toys
  - Hygiene items for care packages
  - Food donation items

## Safety Notes
- Use Service Project Planning Checklist
- Review SAFE Project Tool Use guidelines
- Provide proper training for any tools used
- Ensure continuous adult supervision
- Follow manufacturer guidelines for craft materials
- Review Digital Safety guidelines if using internet research

## Take-Home Elements
- Scouts may continue collecting items for ongoing service projects
- Families can visit nonprofit organizations together
- Continue practicing flag etiquette at home

## Extension Opportunities
- Visit local nonprofit organization
- Participate in council-sponsored Scouting for Food
- Coordinate with pack for larger service project
- Connect with chartered organization for service opportunities
